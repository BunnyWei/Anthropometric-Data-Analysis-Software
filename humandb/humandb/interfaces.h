...
{
public:
	//作为MeshIOInterface的内部类主要定义了IO插件的对应的文件的格式以及信息
	class Format
	{
	public:
...
public:
	...
	static QString getBaseDirPath()
	{
		...
		qDebug( "Current Dir: %s ",qPrintable(baseDir.absolutePath()));
		#if defined(Q_OS_WIN)
		// Windows: 
	...
	static QString getPluginDirPath()
	{
		...
		if(!pluginsDir.exists("plugins"))
				QMessageBox::warning(0,"三维人体模型系统初始化","严重错误：找不到加载的插件");
...
public:
  typedef int FilterIDType;
	...
	// 获取对应的Filter的标识号从而确定到底是属于哪个类型的Filter下面
