...
void PickedPointTreeWidgetItem::setItemName(QString itemName){
	setText(5, itemName);
}

void PickedPointTreeWidgetItem::setPoint(vcg::Point3f &intputPoint)
{
...
QString PickedPointTreeWidgetItem::getItemName()
{
	return text(5);
}
...
{ 
	...
	headerNames << "Point Name" << "X" << "Y" << "Z" << "active" << "item" ;

	ui.pickedPointsTreeWidget->setColumnWidth(0, 90);
	ui.pickedPointsTreeWidget->setColumnWidth(1, 90);
	ui.pickedPointsTreeWidget->setColumnWidth(2, 90);
	ui.pickedPointsTreeWidget->setColumnWidth(3, 90);
	ui.pickedPointsTreeWidget->setColumnWidth(4, 40);
	ui.pickedPointsTreeWidget->setColumnWidth(5, 100);
...
void PickPointsDialog::addMoveSelectPoint(Point3f point, CMeshO::FaceType::NormalType faceNormal)
{
	if(currentMode == ADD_POINT)
	{
		...
		if( (templateLoaded && NULL != treeItem ) || (NULL != treeItem && !treeItem->isActive()) )
		{
			...
			QString itemName = ui.parameterComboBox->currentText();
			addTreeWidgetItemForPoint(point, name, faceNormal, true, itemName);
		}
...
void PickPointsDialog::selectOrMoveThisPoint(Point3f point){
	...
	for(unsigned int i = 0; i < pickedPointTreeWidgetItemVector.size(); i++){
...
void PickPointsDialog::addPoint(vcg::Point3f &point, QString &name, bool present, QString itemName)
{
	...
	if(NULL != face)
	{
		addTreeWidgetItemForPoint(point, name, face->N(), present, itemName);
	}
	else
	{
		vcg::Point3f faceNormal;
		addTreeWidgetItemForPoint(point, name, faceNormal, present, itemName);
	}
}

PickedPointTreeWidgetItem * PickPointsDialog::addTreeWidgetItemForPoint(vcg::Point3f &point, QString &name, CMeshO::FaceType::NormalType &faceNormal, bool present, QString itemName)
{
	//QString itemName = ui.parameterComboBox->currentText();
	PickedPointTreeWidgetItem *widgetItem =
			new PickedPointTreeWidgetItem(point, faceNormal, name, present);
	widgetItem->setItemName(itemName);
...
void PickPointsDialog::loadPickPointsTemplate(QString filename)
{
	...
	for(unsigned int i = 0; i < pointNameVector.size(); i++){
		vcg::Point3f point;
		vcg::Point3f faceNormal;
		/*PickedPointTreeWidgetItem *widgetItem = 
...
void PickPointsDialog::setCurrentMeshModel(MeshModel *newMeshModel, GLArea *gla){
	...
	{		
		...
		if(NULL != pickedPoints){
			...
			PickedPoint *point;
			for(unsigned int i = 0; i < pickedPointVector->size(); i++){
				point = pickedPointVector->at(i);
				
				addPoint(point->point, point->name, point->present, point->item);
			}
		...
		if(file.exists()){
			//loadPoints(filename); //该句被注释掉以后，启动编辑点云对话框时不加载保存到文件的点
...
void PickPointsDialog::removeHighlightedPoint(){
	...
	if(NULL != item){
		...
		//从数据库中删除相应的记录
		QSqlQuery query;
		QString sql = "delete from points where name = '";
		sql += pickedItem->getName() + "' and itemName = '";
		sql += pickedItem->getItemName() + "'";
		if (query.exec(sql))
		{
		...
		{
			QMessageBox::warning(this, "提示", "删除失败！", QMessageBox::Ok);
		}
	...
	} else
	{
		...
		QMessageBox::warning(this, "警告", "没有选中点！", QMessageBox::Ok);
	}
}
...
PickedPoints * PickPointsDialog::getPickedPoints()
{
	...
	for(unsigned int i = 0; i < pickedPointTreeWidgetItemVector.size(); i++)
	{
		PickedPointTreeWidgetItem *item =
				pickedPointTreeWidgetItemVector.at(i);		
		pickedPoints->addPoint(item->getName(), item->getPoint(), item->isActive(), item->text(5));
	}
...
void PickPointsDialog::loadPoints(QString filename){
	...
	for(unsigned int i = 0; i < points->size(); i++){
		...
		addPoint(pickedPoint->point, pickedPoint->name, pickedPoint->present, pickedPoint->item);		
	}
...
void PickPointsDialog::askUserForFileAndLoadPoints()
{
	...
	*/
	...
	/*QString filename = QApplication::applicationDirPath() + "/item.pp";
	...
	//显示所有的已编辑的测量项void addPoint(vcg::Point3f &point, QString &name, bool present, QString itemName)
	QSqlQuery query;
	QString sql = "select name, x, y, z, active, itemname from points";
	bool bQuery = query.exec(sql);
	...
	while (query.next())
	{
		...
		point.X() = query.value(1).toString().toFloat();
		point.Y() = query.value(2).toString().toFloat();
		point.Z() = query.value(3).toString().toFloat();
		present = query.value(4).toBool();
		itemName = query.value(5).toString();
		addPoint(point, name, present, itemName);
	}
	redrawPoints();
}
...
void PickPointsDialog::on_groupComboBox_currentIndexChanged(int)
{
...
void PickPointsDialog::on_showItemBtn_clicked()
{
	//清空
	pickedPointTreeWidgetItemVector.clear();
    ui.pickedPointsTreeWidget->clear();
    pointCounter = 0;
	//添加某个已经描的测量项
	QString item = ui.parameterComboBox->currentText();
    QSqlQuery query;
	QString sql = "select name, x, y, z, active, itemname from points where itemName = '";
	sql += item;
	sql += "'";
	bool bQuery = query.exec(sql);
