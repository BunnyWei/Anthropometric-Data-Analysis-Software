...
template <class T> void Sphere3<T>::Intersect(const Sphere3<T> &s) {
 ...
 template <class T> int Sphere3<T>::CreateFromBox(int n,  const Point3<T> *points) {
   Point3f max_p(-1e100, -1e100, -1e100);
   Point3f min_p(1e100, 1e100, 1e100);
   for(int i = 0; i < n; i++) {
     for(int k = 0; k < 3; k++) {
       if(max_p[k] < points[i][k]) max_p[k] = points[i][k];
       if(min_p[k] > points[i][k]) min_p[k] = points[i][k];
     }
   }
   Center() = (min_p + max_p)/2;
   Radius() = (min_p - max_p).Norm()/2;
   return 0;
 }
