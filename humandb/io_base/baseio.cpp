...
bool BaseMeshIOPlugin::open(const QString &formatName, const QString &fileName, MeshModel &m, int& mask, const FilterParameterSet &parlst, CallBackPos *cb, QWidget * /*parent*/)
{
  ...
  mask = 0;
	...
	QString errorMsgFormat = "当加载以下文件时候出错 file:\n\"%1\"\n\nError details: %2";
	...
	else 
  {
	...
	QString missingTextureFilesMsg = "以下的纹理文件没有找到\n";
...
bool BaseMeshIOPlugin::save(const QString &formatName,const QString &fileName, MeshModel &m, const int mask, const FilterParameterSet & par, CallBackPos *cb, QWidget *)
{
	QString errorMsgFormat = "导出文件出错 %1:\n%2";
  string filename = QFile::encodeName(fileName).constData ();
  //string filename = fileName.toUtf8().data();
...
{
	if(format.toUpper() == tr("STL"))
		par.addBool("Unify",true, "统一重复顶点",
								"STL格式不是一个点索引文件格式，每一个三角形由独立的点构成。因此通常重复的顶点会被统一成一个顶点.");		
}
void BaseMeshIOPlugin::initSaveParameter(const QString &format, MeshModel &/*m*/, FilterParameterSet &par) 
{
	if(format.toUpper() == tr("STL") || format.toUpper() == tr("PLY"))
		par.addBool("Binary",true, "二进制编码",
								"选中后则用二进制来保存三角模型，否则用ASCII格式来保存模型");		
}
